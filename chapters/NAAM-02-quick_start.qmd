...
By default, this script will **automatically detect** which virus is in each of your barcode folders by subsampling the raw reads and mapping them against your references.

### Configuration Files

Before running the setup script, ensure you have your `virus_config.yaml` ready. This file defines the reference genomes and parameters for the viruses you are analyzing.
...
- `reference_genome, primer, primer_reference`: Absolute paths to the respective FASTA files. The primer_reference is the reference genome that your primers are based on.
...
- `primer_allowed_mismatch`: The number of mismatches allowed while matching and determining the position of the primers. *(Note: If using a multiplexed "super-primer" FASTA with multiple subtypes, consider increasing this to 4 or 5 to ensure off-target primers are successfully trimmed).*
...
### Optional: Manual Sample Map

If you want to bypass the auto-detection, you can optionally provide a `sample_map.csv` to force the assignment of barcodes to specific viruses.
...
barcode05,mpox
```
*(If multiplexing subtypes, e.g., Dengue 1-4, you can list the same barcode multiple times to run the workflow for each subtype simultaneously).*
...
::: callout-important
Please use absolute paths for the **reads** and **virus_config.yaml** so that they can always be located.
:::
...
- `--sample-map {sample_map.csv}`: Path to your manual CSV map (disables auto-detection).
- `--reads-to-test {number}`: Number of reads to subsample per barcode for auto-detection (Default: 2000).
- `--min-reads {number}`: Minimum number of mapped reads required to confidently assign a virus to a barcode (Default: 50). Barcodes failing this threshold will be bypassed.
...
::: callout-note
Upon completion, your project folder will contain a **result/** directory with subfolders for QC, consensus, variants, and nextclade (if enabled).

Crucially, you will find a **consensus_evaluation.tsv** in the root of the results/ folder. This summary file provides a comprehensive overview of every barcode, detailing the assigned virus, read statistics, QC filtering reasons, consensus coverage (%), and reference identity (%), allowing you to assess the success of your run at a glance!
:::
